...
export function TurnosClient({ turnos: initialTurnos, diasBloqueados: initialBloqueados, pendientes, confirmadosHoy, totalMes }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Turnos de Servicio Técnico</h1>
        <p className="text-sm text-gray-500 mt-1">Gestión de turnos del taller</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-gray-900">{confirmadosHoy}</p>
            <p className="text-xs text-gray-500">Confirmados hoy</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-gray-900">{totalMes}</p>
            <p className="text-xs text-gray-500">Total este mes</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border p-5">
            ...
            return (
              <button
                ...
                className={`relative rounded-lg p-1.5 text-sm text-center transition-colors min-h-[44px] flex flex-col items-center justify-start pt-2
                  ${isBloqueado ? "bg-red-50 hover:bg-red-100" : "hover:bg-gray-50"}
                  ${isToday ? "ring-2 ring-[#6B4F7A]" : ""}
                  ${isLoading ? "opacity-50" : ""}
                `}
              >
                <span className={`text-sm font-medium leading-none
                  ${isBloqueado ? "text-red-500" : isToday ? "text-[#6B4F7A] font-bold" : "text-gray-700"}
                `}>
                  {day}
                </span>
                {isBloqueado && (
        ...
        {bloqueados.length > 0 && (
          <div className="mt-4 pt-4 border-t flex flex-wrap gap-2 items-center">
            <span className="text-xs text-gray-500">Días bloqueados:</span>
            {bloqueados
        ...
        {turnosFiltrados.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {turnosFiltrados.map(turno => {
              ...
              return (
                <div key={turno.id} className="bg-white rounded-xl border p-4 space-y-2">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-semibold text-sm text-gray-900">{turno.nombre}</p>
